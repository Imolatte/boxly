...
| E19 | ✅ | Level-up overlay: было захардкожено `+5 к cap`, реально `LEVEL_UP_CAP_BONUS=2`. Прокинул `capBonus` через `showLevelUp(level, energyBonus, capBonus)` → `ui.levelUp.capBonus`. Мульти-апы суммируются (двойной = +4). |
| E20 | ✅ | Продажа коллекционок: `sellValue('collectible')` теперь 5 ⚡ (было `null`). Дубликат-мёрдж даёт +50 ⚡ +2000 XP — продажа дешевле в 10×, мёрдж по-прежнему сильно выгоднее. SellButton показывает `Продать → +5 ⚡` для коллекционок. |
| E21 | ✅ | URL-гранты: `?grant_energy=N&grant_id=<unique>` при загрузке начисляет +N ⚡ (clamp к cap), помечает `grant_id` в `localStorage:boxly_claimed_grants`, чистит query-params. Использовать для одноразовых бонусов друзьям без бэкенд-инфры. |
...
- **URL-bump для обхода кэша TG WebView:** после значимых изменений меняем URL menu-button бота на `?v=N+1`:
  ...
  ```
  Последняя — `?v=25`. Также в `index.html` стоят `no-store` мета-теги чтобы следующие билды не застревали.
...
- `src/store/gameStore.ts` — вся игровая механика. В конце файла `applyUrlGrant()` (E21).
- `src/store/persistence.ts` — `SAVE_VERSION = 4`. Bump для сброса всех.
- `src/engine/sell.ts` — `sellValue` (collectible = 5, complete = 1, остальное 0)
