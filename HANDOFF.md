...
- При драге на `body.dnd-dragging` глобальные правила (см. `src/index.css`):
  - `animation: none !important` на всех с inline animation
  - `backdrop-filter: none !important` на элементах с inline backdrop
  - `transition: none`, `box-shadow: none`, `outline: none` на `.boxly-cell`/`.boxly-sprite`
...
- **Endpoints:**
  - `POST /api/leaderboard-submit` — `{initData, level, xpTotal}` → `ZADD boxly:leaderboard` + `HSET boxly:names`
  ...
  - `POST /api/telegram` — webhook для `/start`, `/play`
- **Score:** `level * 1_000_000_000 + xpTotal` (сортировка по lvl primary, xp secondary)
- **Storage:** Upstash Redis через Vercel Marketplace, free tier (500k команд/мес, хватит x100). ENV переменные: `KV_REST_API_URL` и `KV_REST_API_TOKEN` (автодобавлены интеграцией).
...
- **URL-bump для обхода кэша TG WebView:** после значимых изменений меняем URL menu-button бота на `?v=N+1`:
  ...
  ```
  Последняя — `?v=15`. Также в `index.html` стоят `no-store` мета-теги чтобы следующие билды не застревали.
...
- **Лёгкие лаги драга на Android** (Xiaomi) — уже сильно снижены (memo+containment+native emoji), но не идеально. Further fixes: убрать framer-motion у спрайтов, CSS keyframes вместо spring.
- **Mini App не грузится на мобильных операторах РФ без VPN** — Telegram WebView идёт мимо MTProto-прокси, система DNS/TLS, оператор режет ClientHello по SNI `github.io`. Потенциальное решение: миграция на Яндекс Object Storage (в белом списке Минцифры). Пока все друзья играют через VPN.
- **Leaderboard без HMAC** — теоретически можно подделать через curl с чужим user.id. Для MVP с друзьями ОК, но перед публичным запуском нужен Ed25519 (public key Telegram prod: `e7bf03a2fa4602af4580703d88dda5bb59f32ed8b02a56c187fe7d34caed242d` hex, Web Crypto в Vercel Edge поддерживает).
